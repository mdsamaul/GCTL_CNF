...
{
    public class VendorPrefixController : BaseController
    {
        private readonly IVendorPrefixService _service;

        public VendorPrefixController(ITranslateService translateService, IUserProfileService userProfileService, IVendorPrefixService service) : base(translateService, userProfileService)
        {
            _service = service;
        }
        ...
        public IActionResult VendorPrefixForm()
        {
            return PartialView();
        }

        public IActionResult VendorPrefixList()
        {
            return PartialView();
        }

        [HttpGet]
        [Route("vendor-prefix-list")]
        public async Task<IActionResult> GetAllPaginated(int pageNumber = 1, int pageSize = 10, string searchTerm = "", string sortColumn = "VendorPrifixId", string sortOrder = "desc")
        {
            try
            {
        ...
        public async Task<ActionResult> Details(int id)
        {
            try
            {
                ...
                if (data == null)
                    return NotFound(new { message = $"Vendor Prefix with ID {id} not found." });
        ...
        [HttpPost]
        [Route("vendor-prefix")]
        public async Task<ActionResult> Create([FromBody]VendorPrefixVM model)
        {
            try
            {
                if (model == null)
                    return BadRequest(new { message = "Vendor Prefix is required." });
        ...
        [HttpPut]
        [Route("vendor-prefix/{id}")]
        public async Task<IActionResult> Edit(int id, [FromBody] VendorPrefixVM model)
        {
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                ...
                return Ok(new { message = "Vendore Prefix deleted successfully." });
            }
            catch (Exception ex)
            {
                return StatusCode(500, new { message = "An error occurred.", error = ex.Message });
        ...
        [HttpPost]
        public async Task<IActionResult> CheckDuplicate([FromBody] VendorPrefixVM model)
        {
