...
{
    public class SalesDefPaymentModeController : BaseController
    {
        private readonly IPaymentMode _service;
        public SalesDefPaymentModeController(ITranslateService translateService, IUserProfileService userProfileService, IPaymentMode service) : base(translateService, userProfileService)
        {
            _service = service;
        }
        ...
        public IActionResult PaymentModeForm()
        {
            return PartialView();
        }
        public IActionResult PaymentModeList()
        {
            return PartialView(); 
        }

        [HttpGet("next-payment-mode-id")]
        public async Task<string> GenerateNewIdAsync()
        {
            var lastId = await _service.GetLastPaymentModeAsync();
        ...
        [HttpPost]
        [Route("payment-mode/check-duplicate")]
        public async Task<IActionResult> CheckDuplicate([FromBody] PaymentModeVM model)
        {
        ...
        public async Task<IActionResult> GetAllPaginated(int pageNumber = 1, int pageSize = 10, string searchTerm = "", string sortColumn = "PaymentModeID", string sortOrder = "desc")
        {
            try
            {
        ...
        [HttpPost]
        [Route("payment-mode")]
        public async Task<ActionResult> Create([FromBody] PaymentModeVM model)
        {
            try
            {
        ...
        public async Task<IActionResult> Edit(int id, [FromBody] PaymentModeVM model)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            if (!typeExists)
                return NotFound(new { message = $"Payment Mode with ID {id} not found." });
