...
$('#localizationSettings-pageSizeSelect').on('change', function () {
...
$(document).ready(function () {
    ...
    $("#localizationSettings-searchInput").on("input", function () {
        currentPage = 1;
        loadTableData();
    });

    $("#localizationSettings-prevPageBtn").on('click', function () {
        if (currentPage > 1) {
            currentPage--;
            loadTableData();
        }
    ...
    $("#localizationSettings-nextPageBtn").on('click', function () {
        currentPage++;
        loadTableData();
    });
});
...
function loadTableData(sortColumn, sortOrder) {
    var searchTerm = $("#localizationSettings-searchInput").val();
    $.ajax({
        url: '/LocalizationSettings/GetAllData',
        method: 'GET',
        data: {
        ...
        success: function (response) {
            var tableBody = $("#localizationSettings-tBody");
            tableBody.empty();
                    ...
                    tableBody.append(`
                        <tr class="position-static">
                            <td class="text-center text-middle align-middle" style="width: 5%;">
                                <input type="checkbox" class="form-check-input addHolidayConfig-selectItem" data-id="${item.localizationID}" />
                            </td>
                            <td class="align-middle text-center white-space-nowrap pe-4">${rowIndex}</td>
                             ...
                             <td class="align-middle white-space-nowrap ps-4">${item.languageName}</td>
                             <td class="align-middle white-space-nowrap ps-4">${item.timezoneName}</td>
                             <td class="align-middle white-space-nowrap ps-4">${item.dateFormat}</td>
                             <td class="align-middle white-space-nowrap ps-4">${item.timeFormat}</td>
                             <td class="align-middle white-space-nowrap ps-4">${item.currencyName}</td>
            ...
            $("#localizationSettings-paginationInfo").text(`Showing ${paginationInfo.startItem} to ${paginationInfo.endItem} Items of ${paginationInfo.totalItems}`);
            $("#localizationSettings-totalCount").text(`(${paginationInfo.totalItems})`);
...
function updatePagination(pageNumbers, currentPage, totalPages) {
    const paginationLinks = $("#localizationSettings-paginationLinks");
    ...
    $("#localizationSettings-prevPageBtn").prop('disabled', currentPage === 1);
    $("#localizationSettings-nextPageBtn").prop('disabled', currentPage === totalPages);
}
